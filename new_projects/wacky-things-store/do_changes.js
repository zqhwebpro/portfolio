...
// 1. Remove corner button and game
const luckStart = html.indexOf('<!-- 3.4 PRESS YOUR LUCK MODAL -->');
const luckEnd = html.indexOf('<!-- 3.5 MAIN BLOG LAYOUT (The Mischief Gazette & Wacky Chronicles) -->');
if (luckStart !== -1 && luckEnd !== -1) {
    html = html.substring(0, luckStart) + html.substring(luckEnd);
}

const cornerStart = html.indexOf('<!-- CORNER TRIANGLE MINIGAME BUTTON (SPIN & WIN) -->');
const cornerEnd = html.indexOf('</body>');
if (cornerStart !== -1 && cornerEnd !== -1) {
    html = html.substring(0, cornerStart) + html.substring(cornerEnd);
}

// 2. Add backend panel button in header
const headerBlogBtnStr = '<button onclick="window.toggleViewStoreBlog()" id="header-blog-btn"';
const adminBtnStr = `
                <!-- Admin Panel Trigger -->
...
`;
if (html.includes(headerBlogBtnStr) && !html.includes('id="header-admin-btn"')) {
    html = html.replace(headerBlogBtnStr, adminBtnStr + '\n' + headerBlogBtnStr);
}

// Add admin panel view in main body
const blogMainEnd = html.indexOf('</main>', html.indexOf('id="blog-main-view"')) + 7;
...
if (!html.includes('id="admin-main-view"')) {
    html = html.substring(0, blogMainEnd) + adminPanelHTML + html.substring(blogMainEnd);
}

// 3. Add newsletter signup above footer
...
`;
if (footerStart !== -1 && !html.includes('NEWSLETTER SIGNUP')) {
    html = html.substring(0, footerStart) + newsletterHTML + html.substring(footerStart);
}
...
const adminJS = `
        ...
        window.renderAdminPosts = function() {
            ...
            window.BLOG_POSTS.forEach((post, index) => {
                ...
                el.innerHTML = \`
                    ...
                    <div>
                        <button onclick="window.editPost(\${index})" class="px-4 py-2 bg-yellow-400 text-earth-900 font-bold rounded mr-2">Edit</button>
                        <button onclick="window.deletePost(\${index})" class="px-4 py-2 bg-red-500 text-white font-bold rounded">Delete</button>
                    </div>
                \`;
        ...
        window.editPost = function(index) {
            ...
            // Overwrite delete and create a new post flow
