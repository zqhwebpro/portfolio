<?php
// get ruhuman class
require WEBROOT . 'lib/js/ruhuman/ruhuman.class.php';
...
if (!empty($_POST)) {
    ...
    if (!ruhuman::is_human()) {
        ...
        $rarr['errors'][] = 'You don\'t appear to be a human.<br>Please refresh this page and try again.';
        die(json_encode($rarr));
    }

    require WEBROOT . 'lib/inc/spamBeast/spamBeast.class.php';
    ...
    require 'lib/inc/PHPMailer/PHPMailerAutoload.php';
    ...
    $mail->setFrom('no-reply@weberadvertising.com', 'Buckland Insurance Agency, Inc.');
    $mail->addReplyTo($_POST['quoteEmail'], $_POST['quoteName']);
    // decide who to send the email to based on spam score
	if ($spam_score <= 4) {
        $mail->addAddress('info@bucklandinsurance.com');
        $mail->addAddress('jennym@bucklandinsurance.com');
		$mail->Subject = '[Buckland Insurance Agency, Inc.] Quote Request - Discover The Power';
	} else {
		$mail->addAddress('vyeaglin@weberadvertising.com', 'Vera Yeaglin');
		$mail->Subject = '[MARKED][Score: ' . $spam_score . '][Buckland Insurance Agency, Inc.] Quote Request - Discover The Power';
	}
