...
*/
$(function() {
    $('#ic_career_edit').on('show.bs.modal', function(e) {
        ...
        if (cur_id) {
                ...
                $('input, textarea, select', the_form).filter('[name]').not(':button, :submit, :file').each(function(i, item) {
                    ...
                    if (data.data[$(item).attr('name')]) {
                        ...
                        if ($(item).hasClass('tinymce')) {
                            tinymce.get($(item).attr('id')).setContent(data.data[$(item).attr('name')]);
                        }
                    }
                });
            })
            ...
            $('#ic_career_edit input[name=id]').val('');
            // set default qualifications
        }
    });
    $('#ic_career_edit').on('shown.bs.modal', function() {
        $('input, textarea, select', this).filter(':visible').first().focus();
    });

    $('#ic_career_delete').on('show.bs.modal', function(e) {
    ...
    $('#ic_career_edit form, #ic_career_delete form').on('submit', function(e) {
        ...
        var api_url = 'api/careers/';
        var api_method = 'PUT';
        var success_message = 'Career successfully created';

        if (cur_id) {
            api_url += '?id=' + cur_id;
            // check if we are deleting or patching
            if (to_close[0].id == 'ic_career_delete') {
                api_method = 'DELETE';
                success_message = 'Career successfully deleted';
            } else {
                api_method = 'PATCH';
                success_message = 'Career successfully updated';
            }
        }
