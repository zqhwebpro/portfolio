...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        if (!empty($_GET['id'])) {
            ...
            $item = $db->query('SELECT * FROM ' . DATABASE_TABLE_GROUPS . ' WHERE `id`=:iid;', array(':iid' => $_GET['id']), null, true);
            // make sure we have an item to return
            if (empty($item)) {
                header('HTTP/1.1 404 Not Found', true, 404);
                $rarr['errors'][] = 'Group does not exist';
                die(json_encode($rarr));
            }
            // add item to return data
            $rarr['data'] = $item;
        } else {
            $rarr['data'] = $db->query('SELECT * FROM ' . DATABASE_TABLE_GROUPS . ' ORDER BY `grp`;');
        }
        break;
    case 'PATCH':
        ...
        $item = $db->query('SELECT * FROM ' . DATABASE_TABLE_GROUPS . ' WHERE `id`=:iid', array(':iid' => $_GET['id']), 'dbRow', true);
    ...
    case 'PUT':
        ...
        // get the item we are working with
        $item = new dbRow($db->handle, DATABASE_TABLE_GROUPS);
