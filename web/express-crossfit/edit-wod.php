  ...
  <div class="wrap">
	...
	<nav class="group" id="ec_nav">
		...
		<div class="content_group">
			<div class="group">
				<div class="grid_6_of_6">
					<h1 id="multipage_hl">Edit WoD</h1>
				</div>
			</div>
		</div>
		<div id="admintop_group">
			...
			<div class="group">
				<div class="grid_6_of_6">
					<?php $sql=("SELECT * FROM `wod` WHERE `wodid`= '".$_GET['wodid']."'");
					        echo $sql;
					        $result=mysqli_query($con, $sql); ?><?php while($row=mysqli_fetch_array($result)){ ?><?php if(isset($msg)){echo $msg;}?>
					<form action="%3C?php%20echo%20$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'];?%3E" class="ec_form" method="post">
						...
						<div>
							<label for="addedby">Added By:</label> <input class="" name="addedby" type="text" value="<?php echo $row['addedby'];?>">
						</div>
						<div>
							<label for="addedon">Added On:</label> <input class="" name="addedon" type="text" value="<?php echo $row['addedon'];?>">
						</div>
						<div>
							<label for="imageid">ImageID:</label> <input class="" name="imageid" type="text" value="<?php echo $row['imageid'];?>">
						</div>
						<div>
							<label for="wodid">WoDID:</label> <input class="" name="wodid" type="text" value="<?php echo $row['wodid'];?>">
						</div>
						<div>
							<label for="wodname">wodname:</label> <input class="" name="wodname" type="text" value="<?php echo $row['wodname'];?>">
						</div>
						<div>
							<label for="wodtext">WoD Text:</label> <input class="" name="wodtext" type="text" value="<?php echo $row['wodtext'];?>">
						</div>
						<div>
							<input class="" name="submit" type="submit" value="Update">
						</div>
					</form><?php } ?><?php include('inc/basic-upload.php'); ?><br>
